...
def generate_rule_with_openai(user_input: str, client_id: Optional[int] = None):
    ...
    system_prompt = f"""You are a rule generation assistant. Create rules based on this confirmed logical structure:
        ...
        ### CRITICAL CONNECTOR RULES FOR RULE GENERATION:
        When generating JSON rules, follow these EXACT connector rules:

        1. **Between individual conditions in a group**: Use "connector" field
           - First condition in a group gets "connector": "AND" or "connector": "OR" based on group's connector
           - Last condition in a group gets NO "connector" field (or null)
           - Example in group with OR: first condition has "connector": "OR", second has no connector

        2. **Inside condition groups**: Use "groupConnector" field for the ENTIRE group
           - If group uses OR logic: "groupConnector": "OR"
           - If group uses AND logic: "groupConnector": "AND"

        3. **Between top-level items (conditions or groups)**: Use "connector" field
           - First top-level item gets "connector": "AND" or "connector": "OR" based on topLevelConnector
           - Last top-level item gets NO "connector" field (or null)

        4. **Top level**: Always include "topLevelConnector": "AND" or "topLevelConnector": "OR"

        If it is (A AND B) OR C, Output JSON matching this EXACT schema with correct connectors:
        {{
            "rules": [
                {{
                    "id": <id>,
                    "priority": null,
                    "ruleType": "conditionGroup",
                    "groupConnector": "AND",  // Inside the group, use AND
                    "conditions": [
                        {{
                            "id": <id>,
                            "dataSource": <data source name>,
                            ...
                            "operator": <operator>,
                            "value": "2500",
                            "connector": "AND"  // Connects to next condition IN THE GROUP
                        }},
                        {{
                            "id": <id>,
                            "dataSource": <data source name>,
                            ...
                            "operator": <operator>,
                            "value": "1000"
                            // NO connector - last condition IN THE GROUP
                        }}
                    ],
                    "connector": "OR"  // Connects this GROUP to next top-level item
                }},
                {{
                    "id": <id>,
                    "dataSource": <data source name>,
            ...
            "topLevelConnector": "OR"  // Top level uses OR
        }}
        ...
        {{
            "rules": [
                {{
                    ...
                    "priority": null,
                    "ruleType": "condition",
                    "connector": "AND"  // Connects this condition to next top-level item
                }},
                {{
                    "id": <id>,
                    "priority": null,
                    "ruleType": "conditionGroup",
                    "groupConnector": "OR",  // Inside the group, use OR
                    "conditions": [
                        {{
                            "id": <id>,
                            "dataSource": <data source name>,
                            ...
                            "operator": <operator>,
                            "value": "active",
                            "connector": "OR"  // Connects to next condition IN THE GROUP
                        }},
                        {{
                            "id": <id>,
                            "dataSource": <data source name>,
            ...
            "topLevelConnector": "AND"  // Top level uses AND
        }}
        ...
        6. Follow the logical structure EXACTLY as provided
        7. When generating eligibilityPeriod:
           If the condition explicitly mentions a **date range** (e.g., "between February and March", "from 2025-02-01 to 2025-03-31", "between 9 AM and 6 PM"), set eligibilityPeriod = 'n_a'.
           If the condition mentions 'last month" or "up to this month', set eligibilityPeriod to "Last X days" (e.g., "Last 30 days").
           If the condition mentions 'every month', set eligibilityPeriod to "Rolling X days".
           Never swap these values.
           Replace X with the configured number of days.
        8. For amount aggregations, use "sum" function
        9. **MOST IMPORTANT**: Follow the connector rules EXACTLY as shown in the examples above
